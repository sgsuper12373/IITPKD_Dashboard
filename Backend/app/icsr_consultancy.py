...
logo upload that CSV cannot express. Logos are stored under uploads/industry/,
mirroring the IPTIF facilities / startup portfolio flow.
"""
import os
import uuid
...
def _save_image(file):
    ...
    upload_folder = current_app.config.get(
        'INDUSTRY_UPLOAD_FOLDER',
        os.path.join(os.path.dirname(__file__), '..', 'uploads', 'industry'),
    )
...
def _delete_image_file(logo):
    ...
    upload_folder = os.path.join(os.path.dirname(__file__), '..', 'uploads', 'industry')
    try:
