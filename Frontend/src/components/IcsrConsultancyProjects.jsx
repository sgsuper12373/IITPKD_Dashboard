...
function LogoThumb({ project, size = 44 }) {
  ...
  if (src) {
    return <img src={src} alt={project.sponsoring_industry || 'Industry'} className="icp-logo" loading="lazy" />;
  }
  return (
...
function ProjectForm({ initial, isCreate = false, onSave, onCancel }) {
  ...
  const [form, setForm] = useState({
    ...
    sponsoring_industry: initial?.sponsoring_industry ?? '',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.principal_investigator.trim() || !form.department.trim()) {
      setError('Principal investigator and department are required.');
      return;
    }
    setSaving(true);
    setError('');
    ...
    >
      <div className="icp-form-panel">
        <h3 className="icp-form-title">
          {isCreate ? 'Add Consultancy Project' : `Edit “${initial?.project_title || 'project'}”`}
        </h3>
        <form onSubmit={handleSubmit} className="icp-form">
          <label className="icp-form-label">
            ...
            <input className="icp-form-input" value={form.project_title}
              onChange={setField('project_title')} placeholder="e.g. Structural analysis for …" autoFocus />
          </label>

          <div className="icp-form-grid">
            <label className="icp-form-label">
              Principal investigator<span className="icp-req">*</span>
            ...
            <label className="icp-form-label">
              Department<span className="icp-req">*</span>
              <input className="icp-form-input" value={form.department}
                onChange={setField('department')} placeholder="e.g. Mechanical Engineering" />
            </label>
          </div>
          ...
          <div className="icp-form-grid">
            <label className="icp-form-label">
              Sponsoring industry
              <input className="icp-form-input" value={form.sponsoring_industry}
                onChange={setField('sponsoring_industry')} placeholder="e.g. Tata Steel Ltd." />
            </label>
            <label className="icp-form-label">
              Project area
...
function IcsrConsultancyProjects({ user }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setProjects((await fetchConsultancyProjects(token)) ?? []);
      setError('');
    } catch (err) {
      setProjects([]);
  ...
  const handleEdit = async (formData) => {
    await updateConsultancyProject(editing.project_id, formData, token);
    setEditing(null);
    await load();
  };

  const handleAdd = async (formData) => {
    await createConsultancyProject(formData, token);
  ...
  const visible = useMemo(() => {
    ...
    return projects.filter((p) =>
      `${p.project_title ?? ''} ${p.principal_investigator ?? ''} ${p.department ?? ''} ${p.sponsoring_industry ?? ''}`
        .toLowerCase()
        .includes(q),
    );
  }, [projects, search]);
    ...
    return (
      <div className="page-container">
        <div className="page-content">
          <p className="icp-empty">You don’t have access to manage consultancy projects.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="page-container">
      <div className="page-content">
        ...
        <div className="icp-header">
          <div>
            <h1 className="icp-h1">Consultancy Projects</h1>
            <p className="icp-subtitle">
              Upload, edit, and attach sponsoring-industry logos for ICSR consultancy projects.
            </p>
            <LastUpdated tables={['icsr_consultancy_projects']} />
          </div>
        ...
        {loading && !hasLoaded ? (
          ...
          <p className="icp-empty">
            {projects.length === 0
              ? 'No consultancy projects yet. Use “Upload CSV” or “Add Project” to get started.'
      ...
      <DataUploadModal
        ...
        tableName="icsr_consultancy_projects"
        token={token}
...
export default IcsrConsultancyProjects;
